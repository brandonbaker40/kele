...
class Kele
  ...
  def create_submission(checkpoint_id, assignment_branch, assignment_commit_link, comment)
    response = self.class.post(@api_url + "/checkpoint_submissions", body: {"assignment_branch": assignment_branch, "assignment_commit_link": assignment_commit_link, "checkpoint_id": checkpoint_id, "comment": comment, "enrollment_id": 24370}, headers: { "authorization" => @auth_token })
    @create_submission = response["enrollment_id"]
  end
